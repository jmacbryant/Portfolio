...
//					panel.setPreferredSize( new Dimension( width, height ) );
	...
	public void animate( float[] samples, final float samplesPerPixel, final Color color)
	{			
		...
		{		
			...
			if( image.getWidth() <  samples.length / samplesPerPixel)
			{
				...
				for( int i = 1; i < samples.length; i+=2 )
				{
					...
					g.fillRoundRect( (int)(i-1), image.getHeight() - ((int)(samples[i-1] * samplesPerPixel)/2), (int)(i+1), (int)(samples[i-1] * samplesPerPixel)/2, 10, 10);
